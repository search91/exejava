 ...
 * @date 2017年02月16日
 */
public class RefineSubject extends UDF{
	...
	private String shortestKeywordHandle(String subject,
		...
		if (domain !=null && !domain.trim().isEmpty()) {
			...
			for (int i=0;i<domains.length;i++) {
				if (domains[i].length()>4)
					domainList.add(domains[i].trim().toLowerCase());
			}
		}
		keywordList2.addAll(domainList);
		...
		for (String keyword : keywordList2) {
			...
			if (tmpStartIdx!=-1) {
				...
				else if (!domainList.contains(keyword)) //排除domain
	...
	private String specialHandle(String subject, String domain) {
		...
		// birchstreet.net
		if (domain.equals("birchstreet.net") && (subject.toLowerCase().contains("a purchase order") || subject.toLowerCase().contains("apurchaseorder"))) {
		...
		// 处理采购
		if (domain.equals("vip.chinabidding.com.cn") && subject.startsWith("采购与招标网[关键字：")) {
			return "采购与招标网[关键字：";
		}
		// 金立手机
		if (domain.equals("gionee.com") && subject.contains("验证")) {
			return "验证";
		}
		
		if (domain.equals("alert-ci.oneapm.com") && subject.toLowerCase().trim().startsWith("【Cloudinsight 报警通知】")) {
	...
	public static void main(String[] args) {
		RefineSubject subject= new RefineSubject();
		...
		try {
			...
			kws.add("订单");
			kws.add("欢迎");
			System.out.println(subject.evaluate("保单号ABEJ171CTP17X106850K电子发票通知", kws, "cpic.com.cn"));
			System.out.println(subject.evaluate("欢迎使用Payoneer 派安盈！", kws, "payoneer.com"));
			System.out.println(subject.evaluate("保单号ABEJ171CTP17X106850K电子发票通知", kws, "cpic.com.cn"));
			System.out.println(subject.evaluate("出门问问的现场面试邀请 - 苏文利", kws, "cpic.com.cn"));
			System.out.println(subject.evaluate("信用保障3000美金以下订单，可自主选择是否通过一达通出口", kws, "cpic.com.cn"));
			System.out.println(subject.evaluate("基本养老保险a类优先配售地位正式确认", kws, "cpic.com.cn"));
			
			File kwFile = new File("E:\\project\\java\\exeJava\\kw.txt");
			...
			File inputFile = new File("E:\\project\\java\\exeJava\\ttt.txt");
